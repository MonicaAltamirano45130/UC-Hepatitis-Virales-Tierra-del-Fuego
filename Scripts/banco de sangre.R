...
banco_de_sangre <- data %>% filter(`Grupo de evento` == "Banco de Sangre") # Filtramos el evento Banco de sangre

data_columnas_interes <- banco_de_sangre %>% # Selecciono columnas de interés
  select(Evento,`Clasificación manual del caso`,`Sexo Legal`,`Grupo etario diagnóstico`, `Nro Doc`,`Fecha Apertura`)

resumen_eventos <- banco_de_sangre %>% # Resumo y agrupo las clasificaciones manuales
...
resumen_eventos_ordenado_bs <- resumen_eventos %>% 
  arrange(desc(Casos))%>% 
  adorn_totals(where = "row")  # agregamos totales 

# Análisis HV Hepatología ======================================================
# Me falta crear una base de banco de sangre con HV B y C, subirla y unirla a la base de SNVS
...
data_bs_completa <- left_join(data_columnas_interes, banco_de_sangre, by = "Nro Doc") 

colnames(data_bs_completa) #Observo los nombres de las columnas de la base completa
...
data_bs_completa <- data_bs_completa %>%
